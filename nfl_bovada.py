...
def send_request():
    ...
    with Browser('chrome') as browser:
        base_url = "https://sports.bovada.lv/football/nfl"
        ...
        # 
        header = ['game_datetime', 'home', 'away', 'home_spread', 'away_spread']
        day_marker = time.strftime("%Y%m%d")
        file_name = 'sportsplay/nfl_bovada_'+ day_marker +'.csv'

        items = browser.find_by_xpath("//article[contains(@class, 'gameline-layout') and contains(@class, 'row') and contains(@class, 'ng-scope') and contains(@class, 'show-cta')]")

        for key, item in enumerate(items):
            soup = BeautifulSoup(item.html, 'html.parser')
            # get date time
            gameDateTime = soup.find('time').string
            print(gameDateTime)

            h3s = soup.find_all('h3')
            # get home name
            homeName = h3s[0].string
            print(homeName)
            # get away name
            awayName = h3s[1].string
            print(awayName)

            spans = soup.find_all('span', {'class': ['ng-binding', 'ng-scope']})
            # get home spread
            homeSpread = spans[2].string.replace(u'\xa0', ' ').replace(u'\xbd', '.5')
            print(homeSpread)

            # get away spread
            awaySpread = spans[4].string.replace(u'\xa0', ' ').replace(u'\xbd', '.5')
            print(awaySpread)

            row_values = [gameDateTime, homeName, awayName, homeSpread, awaySpread]
