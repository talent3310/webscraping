...
def soup_url(type_of_line, tdate=str(date.today()).replace('-', ''), driver=''):
    ...
    line_types = [('ML', ''), ('RL', 'pointspread/'), ('total', 'totals/'), ('1H', '1st-half/'),
...
def line_movement_soup(soup, game_date, driver, game_half):
    ...
    f = open('SBR_MLB_' + season + '_' + game_date + '_line_moves.csv', 'a')
    ...
    for num in range(len(eventlines)):
        ...
        with open('sbr/SBR_MLB_' + season + '_' + game_date + '_line_moves.csv', 'a') as f:
...
def get_line_move_data(soup, game_date, game_half, book_name, a_pit_name, h_pit_name, line_change_list=[]):
    ...
    df_line_moves = DataFrame(columns=(
        'Date', 'Book', 'Team', 'Team SP', 'Opponent', 'Opponent SP', 'Full or Half Game?', 'Line Type',
            ...
            try:
                ...
                for consensus_type in consensus_types:
                    ...
                    if (data):
                        ...
                        with open('sbr/mlb_consensus_' + date + '.csv', 'a') as f:
                            for i in rows:
                                f.write(i + '\n')
                time.sleep(1)
                lines += 1
                ...
                print("Error : " + str(eventid) + " " + str(date))
...
def main(driver, season, inputdate=str(date.today()).replace('-', '')):
    ...
    with open('sbr/SBR_MLB_Closing_Lines_' + season + '_' + todays_date + '.csv', 'a') as f:
