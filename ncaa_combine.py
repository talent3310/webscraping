...
def soup_url(type_of_line, tdate=str(date.today()).replace('-', ''), driver=''):
    ...
    url = 'http://www.sportsbookreview.com/betting-odds/ncaa-basketball/' + url_addon + '?date=' + tdate
            ...
            try:
                ...
                for consensus_type in consensus_types:
                    ...
                    if (data):
                        ...
                        with open('sbr/ncaa_consensus.csv', 'a') as f:
                            for i in rows:
                                f.write(i + '\n')
                time.sleep(1)
                ...
                with open('sbr/ncaa_consensus_errors_combine.csv','a') as f:
    ...
    with Browser('chrome') as browser:
        ...
        for key, item in enumerate(items):
            try:
                ...
                for k in browser.find_by_xpath('//div[@id="' + eventid + '"]/div[contains(@class, "el-div") and contains(@class, "eventLine-team")]/div[@class="eventLine-value"]/span[@class="team-name"]'):
                    ...
                    count = count + 1
                # remove ranking from home team
                if (teamName_home_org[0] == '('):
                    pos = teamName_home_org.find(') ') + 2
                    teamName_home_org = teamName_home_org[pos:]
                # remove ranking from away team
                if (teamName_away_org[0] == '('):
                    pos = teamName_away_org.find(') ') + 2
                    teamName_away_org = teamName_away_org[pos:]
        ...
        #         'spread_percentage_sbr', 'spread_percentage_sportsplays'] 
                ...
                for k in range(0, len(result_sportsplay)):
                    ...
                    one_date = one_sportsplay[1]
                    one_time = one_sportsplay[2]
                    one_home = one_sportsplay[7] #sbr home name
                    one_away = one_sportsplay[8] #sbr away name
                    one_percent = one_sportsplay[5]
                    ...
                    #     break
                    if (one_home == teamName_home_org and
                        one_away == teamName_away_org and
                ...
                for k in range(0, len(result_sportsplay)):
                    ...
                    one_date = one_sportsplay[1]
                    one_time = one_sportsplay[2]
                    one_home = one_sportsplay[7] #sbr away name
                    one_away = one_sportsplay[8] #sbr home name
                    one_percent = one_sportsplay[5]
                    ...
                    #     row_away.append(one_percent)
                    #     break
                    if (one_home == teamName_away_org and
                        one_away == teamName_home_org and
                        one_date.find(date) != -1):
                        row_away.append(one_percent)
                        find = 1
                        break
                ...
                for vegas_item in result_vegas:
                    ...
                    print(vegas_date)
                    vegas_spread = vegas_item[10]
                    print(vegas_spread)
                    print(vegas_item[14])

                    if (vegas_item[14] == teamName_home_org and
                ...
                for vegas_item in result_vegas:
                    vegas_date = vegas_item[0] + vegas_item[1]
                    vegas_spread = vegas_item[10]
                    if (vegas_item[14] == teamName_away_org and
                        vegas_date.find(date) != -1):
...
def main(driver, season, inputdate=str(date.today()).replace('-', '')):
    ...
    # with open('sbr/SBR_NCAA_Closing_Lines_' + season + '_' + todays_date + '.csv', 'a') as f:
...
def parse_to_csv(date):
    ...
    for row in rows:
        ...
        if (len(row_values[2:])>0):
            ...
            ################################
            #  get sbr home name
            sbr_home = row_values[3]
            for index in range(len(name_sportsplays)):
                name = name_sportsplays[index]
                if row_values[3] == name:
                    sbr_home = name_sbr[index]
                    break
            ...
            ###########################################
            # get sbr away name
            sbr_away = row_values[4]
            for index in range(len(name_sportsplays)):
                name = name_sportsplays[index]
                if row_values[4] == name:
                    sbr_away = name_sbr[index]
                    break



            row_values_home = [row_values[0], row_values[1], row_values[2], row_values[3],
                                row_values[4], row_values[7], row_values[8], sbr_home, sbr_away]
            row_values_away = [row_values[0], row_values[1], row_values[2], row_values[4],
                                row_values[3], row_values[8], row_values[7], sbr_away, sbr_home]

            print(row_values_home)
            print(row_values_away)
        # file_name = 'sportsplay/ncaa_sportsplay_'+str(day_marker)+'.csv'
        file_name = 'sportsplay/ncaa_sportsplay_combine.csv'
...
def parse_to_csv_vegas(date):
    global result_vegas
    sp = BeautifulSoup(str(soup_url_vegas(date)), 'html.parser')
    # Result list
    a = []
    for uis in g(sp):
        for bt_mouvment_page in get_games_links(uis):
...
if __name__ == '__main__':
    ...
    start_year = input("[SPORTSPLAYS] Please type the year for which you would like to pull data (yyyy):")
    start_month = input("[SPORTSPLAYS] Please type the starting month for which you would like to pull data (1-12):")
    start_day = input("[SPORTSPLAYS] Please type the starting day for which you would like to pull data (1-31):")
    numdays = input("[SPORTSPLAYS] Please type the days for which you would like to pull data (1-365):")
    ...
    # print(result_sportsplay)

    #####################################################
    # get sbr


    season = str(input("[SBR] Please type the year for which you would like to pull data (yyyy):"))
    start_month = input("[SBR] Please type the starting month for which you would like to pull data (1-12):")
    end_month = input("[SBR] Please type the last month for which you would like to pull data (1-12):")
